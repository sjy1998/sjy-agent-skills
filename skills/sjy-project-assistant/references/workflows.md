...
`Deterministic inspect → semantic understanding → ask only missing high-impact facts → recommend sparse collaboration preferences → Owner confirmation → preview → deterministic safe mutation → protocol validation → recommend next workflow.`

Run `scripts/inspect_project.py` for repository facts, then let the LLM interpret those facts and propose only the smallest useful stable collaboration preferences. After confirmation and an exact preview, use `assets/AGENTS.managed-block.md` with `scripts/safe_write.py` for AGENTS mutation and use the PROJECT / STATE templates as their starting structure. Run `scripts/validate_protocol.py` after writing continuity files. Do not turn ordinary questions or read-only orientation into initialization.
...
- **Tools**: when future routing would materially benefit from it and the information remains unknown, ask for expected AI tools, models, or environments plus any durable collaboration preferences or constraints. Executor labels are open and may include coding agents, advisory/web tools, humans, or other project-defined executors. Invite preferences; do not require an exhaustive Responsibility / Executor map.

Ask only for missing high-impact information. When an Empty Greenfield requires a consolidated intake, ask for the missing Project, Next, and Tools in one round and include one short statement that existing requirements, reference projects, prior code, or other materials may be supplied as optional evidence but are not required for Initialize. When the initial request already supplies the needed facts, ask zero redundant questions and do not create a separate interaction merely to deliver the optional-evidence statement. Proceed to a small practical collaboration recommendation only where useful; uncertain Responsibilities remain unmapped. Ask a follow-up only when a remaining material ambiguity would affect correct initialization.
...
`Deterministic reconnaissance → semantic gap analysis → sparse collaboration recommendation → minimal adoption proposal → Owner confirmation → safe managed-block update → minimal PROJECT / STATE write → protocol validation → resume current reality.`
...
The LLM analyzes only Governance, Continuity, and Routing gaps and preserves established practices. When the project lacks mature AI Collaboration guidance and durable preferences would materially help future continuation, ask at most one focused collaboration question after inspecting existing reality, then recommend only useful stable preferences. Preserve existing mature collaboration guidance and do not require an exhaustive Responsibility / Executor map. After confirmation, use `scripts/safe_write.py` for the managed AGENTS block, use the PROJECT / STATE templates as starting structures for minimal writes, and run `scripts/validate_protocol.py`. **Adopt does not force the project back into design/planning.** If evidence establishes that implementation is active, resume implementation without asking the Project Owner to restate the lifecycle stage; if no work is active, record or preserve an Idle state rather than inventing work.
...
`Determine Next Responsibility → applicable PROJECT collaboration preference → Recommended Workflow → short Reason.`

For the current Responsibility, apply explicit Project Owner instruction, STATE current Executor, the applicable PROJECT collaboration preference for the current Responsibility, then the Skill default. When routing to a different next Responsibility, apply explicit Project Owner instruction, the applicable PROJECT collaboration preference for that next Responsibility, the current Executor as fallback when unmapped, then the Skill default. Responsibility and Executor are distinct. PROJECT collaboration guidance is preference, not assignment; STATE Executor records current reality, not permanent ownership. A request for the current Agent/environment to perform or continue the work is explicit Owner instruction when that is the user's intent; asking it to advise about another Executor is not a takeover. Use `superpowers-routing.md` when selecting an engineering method would help.

Do not interrupt active work merely because PROJECT records another collaboration preference. At a natural transition to a different major Responsibility, surface the applicable PROJECT collaboration preference once as an optional recommendation. If the Owner explicitly chooses the current capable environment for that Responsibility, continue there and do not mention that Responsibility’s applicable PROJECT collaboration preference again until that Responsibility completes or changes. Capability mismatch remains mandatory to report and follows `exceptions.md`; the preference may also be discussed again if the Owner asks about it.

When recommending an Executor or durable collaboration preference, keep Executor labels open and consider Project Owner preference, the Responsibility, available tools, and whether each tool in its current usage mode can actually perform the required work. Prefer a repository-capable executor for work that requires direct repository reading, local mutation, terminal execution, testing, Git inspection, or continuous implementation. Advisory or web environments may fit Research, Requirements, strategic planning, technical comparison, high-level architecture discussion, documentation, review, or decision support when the available context is sufficient. Do not assign fixed roles by brand, build a capability registry, or change the authoritative routing precedence.
